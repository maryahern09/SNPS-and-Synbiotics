# Batch job for Regularization Models Part 1- Glucose
...
#load data
data_glucose <- fread("~/Regularized Regressions SNP/GlycogenesSNPs_qc_final.txt")
...
write.table(coef_df, file = "~/Regularized Regressions SNP/EN_glucose.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)

# Write only non-zero coefficients to file
write.table(nonzero_coef_df, file = "~/Regularized Regressions SNP/EN_filtered_glucose.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)
