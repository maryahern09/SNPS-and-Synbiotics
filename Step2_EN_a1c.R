...
#load data
data_a1c <- fread("/GlycogenesSNPs_qc_PLINKandR.csv")
...
x_final <- data_subset %>%
  ...
  scale() %>%
  as.matrix()
y_final <- data_subset$hgb_a1c_glyco
...
write.table(coef_df, file = "/EN_a1c.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)

# Write only non-zero coefficients to file
write.table(nonzero_coef_df, file = "/EN_filtered_a1c.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)
