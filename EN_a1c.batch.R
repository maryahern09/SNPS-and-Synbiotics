# Batch job for Regularization Models Part 1- A1c
...
#load data
data_a1c <- fread("~/Regularized Regressions SNP/GlycogenesSNPs_qc_final.txt")
...
train_indices <- createDataPartition(y = data_a1c$hgb_a1c_glyco, 
                                     p = 0.6, list = FALSE) # 60% for training
train_data <- data_a1c[train_indices, ]
remaining_data <- data_a1c[-train_indices, ]

# Split the remaining data into validation and test sets
# Assuming a 50/50 split of the remaining data (15% validation, 15% test)
validation_indices <- createDataPartition(y = remaining_data$hgb_a1c_glyco, 
                                          p = 0.5, list = FALSE)
...
x_train <- train_data %>%
  ...
  scale()%>%
  as.matrix() 
y_train <- train_data$hgb_a1c_glyco
...
x_validation <- validation_data %>%
  ...
  scale() %>%
  as.matrix()
y_validation <- validation_data$hgb_a1c_glyco
...
x_full_train <- full_train_data %>%
  ...
  scale() %>%
  as.matrix()
y_full_train <- full_train_data$hgb_a1c_glyco
...
x_test <- test_data %>%
  ...
  scale() %>%
  as.matrix()
y_test <- test_data$hgb_a1c_glyco
...
x_final <- data_a1c %>%
  ...
  scale() %>%
  as.matrix()
y_final <- data_a1c$hgb_a1c_glyco
...
write.table(coef_df, file = "~/Regularized Regressions SNP/EN_a1c.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)

# Write only non-zero coefficients to file
write.table(nonzero_coef_df, file = "~/Regularized Regressions SNP/EN_filtered_a1c.txt", 
            sep = "\t", row.names = FALSE, quote = FALSE)
